...
describe Article do
  ...
  context "name is duplicated" do
    let(:model2){ Article.create name:name, type:type, universe_id:universe2_id }
    context "in the same universe" do
      let(:universe2_id){ universe.id }
      it{ expect{model2; model}.to raise_error{|e|
          expect(e).to be_a ActiveRecord::RecordNotUnique }} 
    end
    context "in different universes" do
      let(:universe2_id){ universe2.id }
      it{ expect{model2; model}.to change(Article,:count).from(0).to(2) }
    end
  end
